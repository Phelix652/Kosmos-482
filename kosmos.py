...
# Initialize crash_time and last_position variables
crash_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
last_position = None  # To store the last position of the satellite
...
try:
    ...
    # Set last known position before the crash (you can define a reasonable fallback position)
    last_position = (51.5533, 146.5134)  # Example: the last known position
...
if satellite_data_valid:
    ...
    if last_position:
        ...
        x, y = m(lon, lat)
        ax.scatter(x, y, color='red', edgecolor='black', s=100, zorder=5, label="Last Kosmos 482 Position")
        st.markdown(f"**Last Known Position of Kosmos 482** — Lat: `{lat:.2f}°`, Lon: `{lon:.2f}°`")
